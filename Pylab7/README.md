...
- **Демонстрационный пример (квадратное уравнение и работа с валютами)** — `Demo.py`  
- **Тесты** — `Tests.py`

> Все функции соответствуют требованиям:  
> - `get_currencies` не содержит логирования и реализует строгую обработку ошибок.  
> - Декоратор `@logger` поддерживает как вывод в поток (`sys.stdout`), так и запись в файл через `logging.Logger`.
...
- **Тесты декоратора `@logger`**:  
  ...
  - Корректная обработка исключений (логируются и пробрасываются)
