...
def integrate_asyncPR(a: float, b: float, *, n_jobs: int = 6 , n_iter: int = 1000) -> float:
    """
    Функция приближенно вычисляет площадь под графиком (определенный интеграл) на интервале [a,b]
    Разбивает интервал на n_jobs подинтервалов и вычисляет интеграл на каждом параллельно, используя Cython версию.
    Args:
        a - левая граница
        b - правая граница
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        spawn = partial(executor.submit, integrate_cython, n_iter=n_iter // n_jobs)
